...
<script>
// Automatisch joinen als ?room= aanwezig is
const urlParams = new URLSearchParams(window.location.search);
const roomParam = urlParams.get('room');
if(roomParam) {
	// Optioneel: roomnaam tonen bovenaan
	const h2 = document.querySelector('h2');
	if(h2) h2.innerText += ' – Room: ' + roomParam;
	if(window.connectRoom) window.connectRoom(roomParam);
}
</script>
</div>
</body>
